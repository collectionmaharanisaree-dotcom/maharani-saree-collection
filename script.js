...
function renderCategoryFilter() { const selected = $('categoryFilter').value || 'All'; $('categoryFilter').innerHTML = '<option value="All">All</option>' + categories.map(c => `<option value="${c.name}">${c.label}</option>`).join(''); $('categoryFilter').value = categories.some(c => c.name === selected) ? selected : 'All'; }
...
function renderCart() { const count = cart.reduce((sum, x) => sum + toNumber(x.qty), 0); $('cartCount').textContent = count; $('cartTotal').textContent = money(cartTotal()); $('cartItems').innerHTML = cart.length ? cart.map(x => { const p = findProduct(x.id); if (!p) return ''; return `<div class="cart-item"><img src="${p.image}" alt=""><div><strong>${p.name}</strong><small>${money(p.price)} · ${p.category}</small><div class="quantity"><button data-change="${p.id}" data-delta="-1">−</button><span>${toNumber(x.qty)}</span><button data-change="${p.id}" data-delta="1">+</button><button class="remove" data-remove="${p.id}">Remove</button></div></div></div>`; }).join('') : `<div class="empty-cart"><span>○</span><p>Your bag is waiting for something beautiful.</p><a href="#shop" id="emptyShop">Explore collection</a></div>`; document.querySelectorAll('[data-change]').forEach(b => b.onclick = () => changeQty(b.dataset.change, b.dataset.delta)); document.querySelectorAll('[data-remove]').forEach(b => b.onclick = () => { cart = cart.filter(x => String(x.id) !== String(b.dataset.remove)); saveCart(); renderCart(); }); const empty = $('emptyShop'); if (empty) empty.onclick = closeDrawer; }
...
async function init() { await loadProducts(); rebuildCategories(); renderCategoryFilter(); $('qrImage').src = `https://api.qrserver.com/v1/create-qr-code/?size=360x360&data=${encodeURIComponent(SITE_URL)}`; $('siteUrl').textContent = SITE_URL; renderCategories(); renderProducts(); renderCart(); $('searchInput').oninput = renderProducts; $('categoryFilter').onchange = renderProducts; $('cartOpen').onclick = openDrawer; $('cartClose').onclick = closeDrawer; $('drawerOverlay').onclick = closeDrawer; $('checkoutOpen').onclick = openCheckout; $('dialogClose').onclick = () => $('productDialog').close(); $('checkoutClose').onclick = () => $('checkoutDialog').close(); $('orderForm').onsubmit = e => { e.preventDefault(); const data = new FormData(e.target); const lines = cart.map(x => { const p = findProduct(x.id); if (!p) return ''; return `• ${p.name} (${p.category}) × ${toNumber(x.qty)} = ${money(p.price * toNumber(x.qty))}`; }).filter(Boolean).join('\n'); const message = `Namaste Maharani Saree Collection!\n\nNew order request\n\nCustomer: ${data.get('name')}\nMobile: ${data.get('mobile')}\nAddress: ${data.get('address')}\nPIN code: ${data.get('pin')}\n\nSelected products:\n${lines}\n\nTotal amount: ${money(cartTotal())}\n\nPlease confirm availability, final price and delivery details.`; window.open(`https://wa.me/${WHATSAPP}?text=${encodeURIComponent(message)}`, '_blank', 'noopener'); }; document.addEventListener('keydown', e => { if (e.key === 'Escape') { closeDrawer(); if ($('productDialog').open) $('productDialog').close(); if ($('checkoutDialog').open) $('checkoutDialog').close(); } }); }
init();
