...
async function renderAdminOrders(){
  ...
  box.querySelectorAll('[data-order-confirm]').forEach(btn=>btn.onclick=async()=>{const o=list[Number(btn.dataset.orderConfirm)];if(!o)return;if(!confirm('इस order को Confirm करके WhatsApp message खोलें?'))return;const {error}=await getSupabaseClient().then(client=>client.from('Orders').update({status:'Confirmed'}).eq('id',o.id));if(error){toast('Confirm failed: '+error.message);return;}lastInvoice=o;saveInvoiceDraft();renderAdminOrders();const lines=(o.items||[]).map(i=>'• '+i.name+' × '+i.qty+' — '+money(Number(i.price||0)*Number(i.qty||0))).join('\\n');const msg='👑 MAHARANI SAREE COLLECTION\\n📍 Derni Bazar, Saran\\n📞 9097900814\\n\\n✅ ORDER CONFIRMED\\n🧾 Order No: '+o.number+'\\n👤 Customer: '+o.name+'\\n📞 Mobile: '+o.mobile+'\\n📍 Address: '+o.address+'\\n📌 PIN: '+o.pin+'\\n\\n📦 ORDER DETAILS\\n'+lines+'\\n\\n💵 Subtotal: '+money(o.subtotal)+'\\n🏷️ Discount: '+money(o.discount)+'\\n💰 TOTAL: '+money(o.total)+'\\n\\n🙏 Aapka order confirm ho gaya hai. Dhanyavaad!';if(o.mobile){window.location.href='https://wa.me/'+String(o.mobile).replace(/\\D/g,'')+'?text='+encodeURIComponent(msg);}});  box.querySelectorAll('[data-order-view]').forEach(btn=>btn.onclick=()=>{lastInvoice=list[Number(btn.dataset.orderView)];renderInvoice();openInvoice();});
